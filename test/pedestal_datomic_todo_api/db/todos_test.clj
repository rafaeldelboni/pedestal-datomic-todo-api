...
(t/deftest ^:integration get-db-test
  (t/testing "should get specific todo"
    (let [{storage :storage} (server/start-system! (build-system-map) system)]
      (let [inserted-todo-1 (todos/create-todo! storage {:text "Play Zelda II"})
            inserted-todo-2 (todos/create-todo! storage {:text "Play RE2 2019"})
...
(t/deftest ^:integration insert-db-test
  (t/testing "should insert a todo"
    (let [{storage :storage} (server/start-system! (build-system-map) system)]
      (let [inserted-todo (todos/create-todo! storage {:text "Play Zelda II"})
            firtst-todo (first (todos/get-todos storage))]
    ...
    (let [{storage :storage} (server/start-system! (build-system-map) system)]
      (let [inserted-todo (todos/create-todo! storage {:text "Play Zelda II"})
            updated-todo (todos/update-todo!
                           ...
                           :done true})
            selected-todo (first (todos/get-todos storage))]
